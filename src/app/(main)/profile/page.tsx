    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center glass-card p-8">
          <p className="text-foreground/60 mb-4">{error || 'No profile data'}</p>
          <div className="flex gap-3 justify-center">
            <Button onClick={() => window.location.reload()} variant="secondary">
              Refresh Page
            </Button>
            <Button onClick={() => signOut({ callbackUrl: '/login' })} variant="danger">
              Logout
            </Button>
          </div>
        </div>
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in-up">
            ...
            <div className="flex-1 w-full md:w-auto text-center md:text-left pt-4 md:pt-20">
               {isEditing ? (
                ...
                <div className="flex flex-col md:flex-row justify-between items-end gap-4">
                  ...
                  <Button variant="secondary" size="sm" onClick={() => setIsEditing(true)} className="shadow-sm border-primary-200 hover:border-primary-300 hover:bg-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        <Card className="p-6 text-center hover:scale-105 transition-transform duration-300 border-primary-100 bg-gradient-to-b from-white to-primary-50/30">
          ...
          <p className="text-sm font-medium text-foreground/50 uppercase tracking-wide mt-1">Total Poin</p>
        </Card>
        ...
        <Card className="p-6 text-center hover:scale-105 transition-transform duration-300 border-blue-100 bg-gradient-to-b from-white to-blue-50/30">
          ...
          <p className="text-2xl md:text-3xl font-bold text-foreground">{profile.totalWaste.toFixed(1)}</p>
          <p className="text-sm font-medium text-foreground/50 uppercase tracking-wide mt-1">kg Sampah</p>
        </Card>
        ...
        <Card className="p-6 text-center hover:scale-105 transition-transform duration-300 border-green-100 bg-gradient-to-b from-white to-green-50/30">
          ...
          <p className="text-2xl md:text-3xl font-bold text-foreground">{profile.treesGrown}</p>
          <p className="text-sm font-medium text-foreground/50 uppercase tracking-wide mt-1">Pohon Ditanam</p>
        </Card>
        ...
        <Card className="p-6 text-center hover:scale-105 transition-transform duration-300 border-orange-100 bg-gradient-to-b from-white to-orange-50/30">
          ...
          <p className="text-sm font-medium text-foreground/50 uppercase tracking-wide mt-1">Hari Streak</p>
        </Card>
      </div>
        ...
        <Card padding="lg" className="h-full">
          ...
          <CardContent>
            <div className="flex items-start gap-4 mb-6">
              <div className="mt-1">
                 <LevelBadge level={userLevel} className="scale-125 origin-top-left" />
              </div>
              <div className="flex-1">
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-foreground/60">Menuju level berikutnya</span>
                  <span className="font-bold text-primary-600">
                    {profile.points.toLocaleString()} / {userLevel === 'BRONZE' ? '1,000' : userLevel === 'SILVER' ? '5,000' : '10,000'}
                  </span>
                </div>
                <div className="h-3 bg-gray-100 rounded-full overflow-hidden shadow-inner">
                  <div 
                    ...
                    style={{ 
                      width: `${Math.min(100, (profile.points / (userLevel === 'BRONZE' ? 1000 : userLevel === 'SILVER' ? 5000 : 10000)) * 100)}%` 
                    }}
                  />
                </div>
                <p className="text-xs text-foreground/40 mt-2">
                   Kumpulkan lebih banyak poin dengan menyetor sampah untuk naik level!
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card padding="lg" className="h-full">
          ...
          <CardContent>
            <div className="flex items-start gap-4">
              <div className="mt-1">
                 <TreeStageBadge stage={treeStage} className="scale-125 origin-top-left" />
              </div>
              <div className="flex-1">
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-foreground/60">Eco XP</span>
                  <span className="font-bold text-green-600">{profile.ecoXp} / 1000</span>
                </div>
                <div className="h-3 bg-gray-100 rounded-full overflow-hidden shadow-inner">
                  <div 
                ...
                <p className="text-xs text-foreground/40 mt-2">
                   Penuhkan bar XP untuk menanam pohon baru dan dapatkan bonus poin.
                </p>
              </div>
            </div>
          </CardContent>
